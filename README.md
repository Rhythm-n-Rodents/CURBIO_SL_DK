...
| `<animal_id>_percentiles_36data.mat` | `processing` >> `signal_percentiles` >> `processing` >> `data` | Stored as a matrix of 2 columns. The first column (str) specifies the signal from which a specific percentile is calculated. For example, `"d_27|pct10"` gives the 10-th percentile of the 27th column (head yaw velocity) in `36data`. The second column stores the corresponding numerical value. |
